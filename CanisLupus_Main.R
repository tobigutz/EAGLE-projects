...
ggplot()+
  ...
  geom_point(data=species3, aes(x=lon, y=lat, colour=year))+
...
writeRaster(gammap$layer, "./GIS/gam_map.tif", format = "GTiff", overwrite = T)
writeOGR(bnd_ger, "./GIS", "Germany_shape", driver="ESRI Shapefile", overwrite = T)
writeOGR(species2, "./GIS", "Canislupus_occ_clean", driver="ESRI Shapefile", overwrite = T)
